...
class CausalSelfAttention(nn.Module):
    def __init__(self, config: GPTConfig):
        ...
        # not really a 'bias', more of a mask, but following the OAI/HF naming tho
    ...
    def forward(self, x):
        ...
        # e.g. in GPT-2 (124M), n_head=12, hs=64, so nh*hs=C=768 channels in a Transformer
        qkv = self.c_attn(x)
...
class GPT(nn.Module):
    def __init__(self, config: GPTConfig):
        ...
        self.transformer = nn.ModuleDict(dict(
            wte = nn.Embedding(config.vocab_size, config.n_embd), # weights token embedding
...
class DataLoaderLite:
    def __init__(self, B, T, process_rank, num_procs):
        self.B = B
        self.T = T
    ...
    def next_batch(self):
        ...
        if self.current_position + (B*T*self.num_procs+1) > len(self.tokens):
            self.current_position = self.B * self.T * self.process_rank
        return x, y
...
train_loader = DataLoaderLite(B=B, T=T, process_rank=ddp_rank, num_procs=ddp_world_size) # on bigger gpus:  16, 1024
...
raw_model = model.module if ddp else model # unwrap
